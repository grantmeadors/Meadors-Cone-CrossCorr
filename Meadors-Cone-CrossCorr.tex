...
\begin{eqnarray}
\delta f - f_0                        &=& s,\\
\delta \Delta f_\mathrm{obs} - \frac{2 f_0 \pi a_p}{P} &=& s \times 1 \times \cos(\theta),\\
\delta t_\mathrm{asc} - t_\mathrm{asc}                   &=& s \times \left(\max\left(\frac{df}{dt} \right)\right)^{-1} * \sin(\theta),
\end{eqnarray}

\noindent The middle equation could equivalently be written

\begin{equation}
\delta a_p - a_p = s \times \frac{P}{2 \pi f_0} \times \cos(\theta).
\end{equation}

Likewise, the equation last expands to

\begin{equation}
\delta t_\mathrm{asc} - t_\mathrm{asc}     = s \times \frac{P^2}{(4 \pi^2 f_0 a_p)} \times \sin(\theta).
\end{equation}
