...
Let $E_f$, $E_a$, $E_t$ be the respective $\epsilon_f$, $\epsilon_{a_p}$, $\epsilon_{t_\mathrm{asc}}$ that individually yield a given $\max (f)$ when the other $\epsilon =0$:

\begin{eqnarray}
\frac{E_a }{ E_f } \sin\phi &=& \frac{P }{2 \pi f_0} \label{errorA}\\
\frac{E_t }{ E_f } \cos\phi &=& \frac{P^2}{4 \pi^2 f_0 a_p}.\label{errorT}
\end{eqnarray}

\noindent Not coincidentally, Equation~\ref{errorA} (when $\sin \phi = 1$) equals the ratio of Equation~\ref{aEqFull} to Equation~\ref{fEq} (when $\sin \theta = 1$).
Analogously, Equation~\ref{errorT} (when $\cos \phi = 1$) equals the ratio of Equation~\ref{tEqFull} to Equation~\ref{fEq} (when $\cos \theta = 1$).
